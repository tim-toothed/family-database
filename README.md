...
- `auto` — сначала попробовать Supabase, а если он недоступен, загрузить локальные YAML;
- `local` — всегда грузить `data/people/index.json` и файлы `data/people/PXXX.yaml`;
- `supabase` — грузить только Supabase и показывать ошибку, если он недоступен.

В режиме `auto` локальный fallback работает так же, как раньше:

1. Сайт пытается прочитать `data/people/index.json`.
...
Для GitHub Pages локальный fallback всё ещё зависит от актуального `data/people/index.json`.
